...
@Observable class Notifications: NSObject, UNUserNotificationCenterDelegate  {
    ...
    func setAlarm(date: Date, id: String = "alarm", sound: String) {
        for i in 0..<10 {
            let content = UNMutableNotificationContent()
            content.title = "Rise and Shine"
            content.body = "Open the app to turn off and set tomorrows alarm"
            content.sound = UNNotificationSound(named: UNNotificationSoundName(rawValue: "\(sound).aif"))
            
            let triggerDate = date.addingTimeInterval(Double(i * 30))
